...
return [
    ...
    /*
    |--------------------------------------------------------------------------
    | Auditoría de calidad (Fase 17): el proyecto no tenía ningún archivo de
    | idioma pese a APP_LOCALE=es — Laravel no trae mensajes de validación
    | en_ni en inglés cargados como fallback interno desde la versión 9+ (hay
    | que publicarlos con `artisan lang:publish`), así que CUALQUIER regla
    | de validación sin mensaje personalizado mostraba la clave cruda
    | ("validation.unique", "validation.required", etc.) en vez de un
    | mensaje legible, en toda la aplicación. Encontrado creando el tenant
    | de demo (Marina Solano) al intentar un slug de tema duplicado.
    |--------------------------------------------------------------------------
    */
    ...
    */
    ...
    |--------------------------------------------------------------------------
    | Nombres de campo en español para que ":attribute" no muestre el nombre
    | crudo en inglés/snake_case de la columna (ej. "name" -> "nombre").
    |--------------------------------------------------------------------------
    */
